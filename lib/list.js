...
class List {
   ...
   * @return {Promise}
   */
	// KNOWN BUG: AnyList's server-side `remove-category` handler currently
	// returns 200 OK without actually applying the operation, regardless of
	// payload shape. Eight plausible variants were tested empirically against
	// the live API and every one was silently dropped (varying handler name,
	// originalValue/originalCategory presence, /update vs /update-v2 endpoint,
	// and operation class).
	//
	// Finding the correct request shape will require wire-level capture from
	// a working client (iOS app via mitmproxy, or anylist.com web app via
	// DevTools). Until that's done, callers MUST treat this method's success
	// return as unreliable, and verify by re-fetching state after the call.
	// AnyListClient.deleteCategory in anylist-mcp does that and throws a clear
	// error if the server didn't actually apply the delete.
