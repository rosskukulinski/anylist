...
class AnyList extends EventEmitter {
   ...
   */
	async getLists(refreshCache = true) {
		...
		// The AnyList user-data/get response splits lists across `newLists`
		// (entries the server thinks are unknown to this client) and
		// `modifiedLists` (entries the server thinks have changed since the
		// client's logical timestamp). For long-lived clients the API drifts
		// toward returning everything in `modifiedLists`, leaving `newLists`
		// empty. Merge both so we always see the complete set, deduping by id.
		const slr = decoded.shoppingListsResponse;
		const seen = new Set();
		const allListRaws = [];
		for (const raw of [...(slr.newLists || []), ...(slr.modifiedLists || [])]) {
			if (raw && raw.identifier && !seen.has(raw.identifier)) {
				seen.add(raw.identifier);
				allListRaws.push(raw);
			}
		}

		this.lists = allListRaws.map(
			list => new List(list, {
				client: this.client,
		...
		const listResponses = slr.listResponses || [];
   ...
   */
	...
	async _getUserData(refreshCache) {
		if (!this._userData || refreshCache) {
			...
			this.calendarId = this._userData.mealPlanningCalendarResponse && this._userData.mealPlanningCalendarResponse.calendarId;
		}
